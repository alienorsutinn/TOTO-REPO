...
class LowCrowd(Strategy):
    ...
    def __init__(
        self, crowd_scores_csv: str, bet_type: str = "BIG", pool_size: int = 1500, seed: int = 0
    ):
    ...
    def generate_bets(self, budget_rm: int) -> list[Bet]:
        ...
        return [
            Bet(number=str(x).zfill(4), bet_type=self.bet_type, stake_rm=1)
            for x in pool["number"].tolist()
        ]
