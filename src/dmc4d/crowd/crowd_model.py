...
def crowd_score(num: str, maps: BirthMaps, weights: dict[str, float] | None = None) -> float:
    ...
    p_dmyy = (maps.p_monthday.get(b.dmyy_md, 0.0) * maps.p_year.get(b.year, 0.0)) if b.dmyy_md else 0.0
    p_mdyy = (maps.p_monthday.get(b.mdyy_md, 0.0) * maps.p_year.get(b.year, 0.0)) if b.mdyy_md else 0.0
...
def score_all_numbers(births_dir: Path, out_path: Path) -> Path:
    ...
    rows = [{"number": f"{i:04d}", "crowd_score": crowd_score(f"{i:04d}", maps)} for i in range(10000)]
